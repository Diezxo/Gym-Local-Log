...
@Component({
  ...
  template: `
    <nav class="fixed bottom-0 left-0 right-0 z-50 bg-[#0a0a0a]/95 backdrop-blur-xl border-t border-[#1e1e1e] pb-safe">
      <div class="flex items-stretch justify-around max-w-lg mx-auto px-1">
        @for (item of navItems; track item.route) {
          <a
            ...
            class="group relative flex flex-col items-center justify-center flex-1 min-h-[68px] py-2 text-[#737373] transition-all duration-300 active:scale-90"
          >
            <!-- Background Pill for Active State -->
            <div
              class="absolute inset-0 m-auto h-10 w-14 rounded-xl bg-cyan-400/10 scale-0 opacity-0 transition-all duration-300 group-[.is-active]:scale-100 group-[.is-active]:opacity-100 -z-10"
            ></div>
            ...
            <span
              class="text-[9px] tracking-wide font-medium transition-colors duration-300 group-[.is-active]:text-cyan-400 group-[.is-active]:font-bold"
            >{{ item.label }}</span>
