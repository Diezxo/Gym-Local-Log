...
@Component({
  ...
  template: `
    <div class="rounded-[32px] bg-[var(--color-bg-card)] p-6 shadow-lg border border-[var(--color-border)] flex flex-col gap-4">
      <!-- Header -->
      <div 
      ...
      <!-- In-Line History -->
      @if (showHistory()) {
        <div class="animate-fade-in bg-[var(--color-bg-input)] rounded-[20px] p-4 border border-[var(--color-border)] shadow-inner">
          <h4 class="text-xs uppercase tracking-wider text-[var(--color-text-muted)] font-bold mb-3">Últimas sesiones</h4>
          @if (history().length > 0) {
            <table class="w-full text-sm text-left">
              <thead>
                <tr class="text-[var(--color-text-muted)] border-b border-[var(--color-border)]">
                  <th class="pb-2 font-bold uppercase tracking-wider text-[10px]">Fecha</th>
                  <th class="pb-2 font-bold uppercase tracking-wider text-[10px]">Mejor Serie</th>
                  <th class="pb-2 font-bold uppercase tracking-wider text-[10px] text-right">Volumen</th>
                </tr>
              </thead>
              <tbody>
      ...
      @if (sugerencia()?.esAlertaCarga) {
        <div class="rounded-[20px] bg-rose-500/15 border border-rose-500/30 p-4 flex items-start gap-3 shadow-[0_0_15px_rgba(244,63,94,0.15)]">
      ...
      <!-- Sets Header -->
      <div class="grid grid-cols-[30px_1fr_64px_64px_50px] gap-2 px-2 mt-4 text-[10px] uppercase tracking-widest text-[var(--color-text-muted)] font-black text-center mb-1">
        <div>Set</div>
      ...
      @if (ejercicioLog().series) {
        @for (serie of ejercicioLog().series; track serie.numero; let isLast = $last) {
          <div 
            class="grid grid-cols-[30px_1fr_64px_64px_50px] gap-2 px-2 py-2.5 items-center text-[var(--color-text-primary)] bg-emerald-500/10 rounded-[16px] mb-2 border border-emerald-500/20 transition-all shadow-[0_0_10px_rgba(16,185,129,0.05)]"
          ...
          >
            ...
            <div class="flex justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#10b981" stroke="none" class="drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
            </div>
          </div>
        }
      }
      ...
      <div class="grid grid-cols-[30px_1fr_64px_64px_50px] gap-2 px-2 items-center mt-3 pb-2">
        ...
        <input
          ...
          class="w-full h-14 rounded-xl bg-[var(--color-bg-input)] border border-[var(--color-border)] text-center font-mono text-lg font-black text-[var(--color-text-primary)] focus:outline-none focus:border-[#00f2fe] focus:ring-1 focus:ring-[#00f2fe]/50 placeholder:text-[var(--color-text-muted)] transition-all shadow-inner"
        />
        ...
        <button
          ...
          [class]="isValidInput() ? 'text-[#00f2fe] hover:bg-[#00f2fe]/10 hover:border-[#00f2fe] hover:shadow-[0_0_15px_rgba(0,242,254,0.3)] cursor-pointer' : 'text-[var(--color-text-muted)] cursor-not-allowed'"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="group-hover:scale-110 transition-transform drop-shadow-[0_0_8px_currentColor]"><polyline points="20 6 9 17 4 12"/></svg>
        </button>
      </div>
    </div>
  `,
